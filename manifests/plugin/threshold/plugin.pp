define collectd::plugin::threshold::plugin (
  ...
  concat::fragment { "collectd_plugin_threshold_conf_threshold_plugin_${title}":
    order   => 50,
    content => template('collectd/plugin/threshold/plugin.conf.erb'),
    target  => "${conf_file}",
  }
